...
#later on need to figure this out.
#unresolved symbol
#import struct
import marshal
...
#import the placer so that we can tell if we are in encrypted mode
import _python
...
class ImportManager:
    ...
    def __init__(self, fs_imp=None):
        ...
        if ( _python.PYImportEncrypedMode() == 1):
            #prefer the encrypted files if they exist
            self.add_suffix('.pye', py_suffix_importer_encrypted)
            self.add_suffix('.py', py_suffix_importer)
        else:
            #prefer the regular .py files 
            #prefer the encrypted files if they exist
            self.add_suffix('.py', py_suffix_importer)
            self.add_suffix('.pye', py_suffix_importer_encrypted)
...
def _compile(pathname, timestamp):
    ...
    #don't try to write it since we don't have the struct package
    return code
...
def _compileString(pathname, codestring, timestamp):
    """Compiles a string from a encrypted Python source file.

    The file specified by <pathname> is is ignored
    returned.
    ...
    # don't try to cache the compiled code
    return code
    try:
        f = open(pathname + _suffix_char, 'wb')
    except IOError:
...
#import an encryped
def py_suffix_importer_encrypted(filename, finfo, fqname):
    file = filename[:-3] + _suffixEncrypted
    t_pye = long(finfo[8])
    code= _python.PYDecryptFileToStringAndCompile(filename)
    return 0, code, { '__file__' : file }
...
def _override_import_with_encrypted_import():
    ImportManager().install()
    sys.path.insert(0, BuiltinImporter())
    #print "finished redefining import"
